...
class StreamingController extends Controller
{
    ...
    public function run() {
        ...
        $videos = Video::whereConfirm(1)->whereNull('link')->whereStatus(-1)->skip(0)->take(5)->get();
        ...
        for ($i = 0; $i < count($videos); $i++) {
            ...
            shell_exec('sshpass -p "ubuntu" scp ' . $filepath . ' root@178.22.123.186:/var/www/koochitaTvVideos/cs/rawFiles/' . $videos[$i]->video);
...
//    $data = array('file' => $cfile, 'time' => $time, 'digest' => $hash, 'videoId' => $videos[$i]->id);
            ...
            curl_setopt($ch, CURLOPT_URL, 'https://show.koochitatv.com/cs/index.php');
